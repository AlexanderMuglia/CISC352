...
class Variable:
    ...
    def assign(self, value):
        ...
        if self.is_assigned() or not self.in_cur_domain(value):
            print(
                "ERROR: trying to assign variable",
                self,
    ...
    def print_all(self):
        print(
            'Var--"{}": Dom = {}, CurDom = {}'.format(self.name, self.dom, self.curdom)
        )
    ...
    def add_satisfying_tuples(self, tuples):
        ...
        for x in tuples:
            ...
            for i, val in enumerate(t):
                var = self.scope[i]
                if (var, val) not in self.sup_tuples:
...
class BT:
    ...
    def bt_search(self, propagator, var_ord=None, val_ord=None):
        ...
        if status == False:
            ...
            status = self.bt_recurse(
                propagator, var_ord, val_ord, 1
        ...
        if not self.QUIET:
            ...
            if status == True:
                print(
                    "CSP {} solved. CPU Time used = {}".format(
                        self.csp.name, time.process_time() - stime
                    )
                )
                self.csp.print_soln()
