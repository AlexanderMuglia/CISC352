...
def check_cage(csp, grid, var):
    ...
    for cage in grid[1]:
        ...
        cage_dict[(f"Cage_op({cage[0]}:{cage[2]}:{in_cage})")] = (
            cage[0],
            in_cage.copy(),
        )
    ...
    for cage in to_be_checked:
        ...
        if cage.assignedValue == "+":
            ...
            if (
                len(cage_dict[cage.name][1]) == 1
                and cage_dict[cage.name][1][0].assignedValue != cage_dict[cage.name][0]
            ):
                print("fail on freecell" + cage.name)
                return False
            elif (
                not add_valid(cage_dict[cage.name][0], assigned_values)
                and not sub_valid(cage_dict[cage.name][0], assigned_values)
                and not mul_valid(cage_dict[cage.name][0], assigned_values)
                and not div_valid(cage_dict[cage.name][0], assigned_values)
            ):
...
class TestBinaryGrid(unittest.TestCase):
    ...
    def test_bne_grid_1(self):
        ...
        if (
            len(constraints_binary) == unidirectional_ans
            or len(constraints_binary) == bidirectional_ans
        ):
            passed = True
...
class TestCageConstraints(unittest.TestCase):
    ...
    def test_cage_existence(self):
        ...
        correct = (
            "Cage_op(6:+:[Var-Cell(1,1), Var-Cell(1,2), Var-Cell(2,1), Var-Cell(2,2)])"
        )
    ...
    def test_cages_1(self):
        ...
        if not isinstance(guess[0].scope[0].dom[0], str):
            ...
            normalized_guess = []
            for g in guess:
                normalized_guess.append(
                    Constraint(g.name, [g.scope[-1]] + g.scope[:-1])
                )
                rev_sat = []
...
def main(verbosity=2):
    ...
    with open(file_name, "a") as out:
        ...
        unittest.TextTestRunner(
            out, verbosity=verbosity, resultclass=NoTraceResult
        ).run(suite)
